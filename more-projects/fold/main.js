...
var previewFold = new Preview({
	geometry: foldFunction(1).getGeometry(),
...
var previewFlat = new Preview({
	geometry: foldFunction(0).getGeometry(),
	width: Math.round(window.innerWidth / 4),
	height: Math.round(window.innerHeight / 2),
	parent: document.body,
	materials: [
...
var previewAnim = new Preview({
	...
	onRender: function(preview) {
		t += 0.01;
		preview.replaceGeometry(foldFunction(Math.sin(t)).getGeometry());
...
function runNewCode(code) {
	...
	previewFold.replaceGeometry(fold.getGeometry());
	foldFunction = main;
	showSuccess('The fold diagram has been updated');
}
